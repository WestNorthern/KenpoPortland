...
const all_techniques = require('../basics.json');


function BasicsListScreen({ navigation }) {
  ...
  const filterTechniquesByType = (type) => {
    // Delete the message from messages
    setTechniques(all_techniques.filter((m) => m.type == type));
  };
  ...
  return (
    <Screen style={styles.screen}>
      <View style={styles.colorBarContainer}>
        ...
        <TouchableWithoutFeedback onPress={() => filterTechniquesByType("Strike")}><Icon style={[styles.searchIcon, styles.colorCircle]} size={28} name="hand"></Icon></TouchableWithoutFeedback>
      </View>
      <FlatList
        data={techniques}
        keyExtractor={(technique) => technique.name.toString()}
        renderItem={({ item }) => (
          <TechniqueListItem
            IconComponent={<TechniqueIcon name="circle" iconColor="black" backgroundColor={beltToColor(item.belt)}  />}
            title={item.name}
            color={item.belt}
            onPress={ () => navigation.navigate(routes.BASICS_DETAILS, item)}
          />
        )}
...
export default BasicsListScreen;